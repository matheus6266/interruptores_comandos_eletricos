...
# 4.1.1 Cálculo da Corrente Média e Corrente RMS:
...
corrente_media <- (ciclo_trabalho/2)*corrente_pico;
sprintf("A corrente média será: %.2f A", corrente_media);
...
```{r}
corrente_rms <- corrente_pico*sqrt(ciclo_trabalho/3);
sprintf("A corrente RMS será: %.2f A", corrente_rms);

```
# 4.2.2 Cálculo Potência Condução:
...
```{r}
calculo_potecia_conducao <- function(tensao_conducao, corrente_media, resistencia_direta, corrente_rms){
...
potencia_conducao <- calculo_potecia_conducao(tensao_conducao_diodo,corrente_media,resistencia_direta_diodo,corrente_rms);
sprintf("A potência de condução direta do diodo será: %.2fW", potencia_conducao);

```
# 4.1.3 Cálculo da Temperatura de Junção:
...
```{r}
calculo_temperatura_juncao <- function(potencia_conducao, resistencia_ja, temperatura_ambiente){
  temperatura_juncao <- (potencia_conducao*resistencia_ja) + temperatura_ambiente;
...
temperatura_juncao <- calculo_temperatura_juncao(potencia_conducao, resistencia_ja, temperatura_ambiente);
sprintf("Temperatura calculada: %.2f ºC, temperatura máxima: %.2f ºC. Desta forma
        ve-se a necessidade de se usar um dissipador de calor.", temperatura_juncao,
        temperatura_maxima_juncao);

```

# 4.1.4 Cálculo do Dissipador de Calor:
...
resistencia_ja_projeto <- calculo_resistencia_ja(temperatura_juncao_projeto,temperatura_ambiente, potencia_conducao);
resistencia_da <- calculo_resistencia_da(resistencia_ja_projeto, resistencia_jc, resistencia_cd);
resistencia_da_final <- 12; # Valor da reistência do dissipador do catálogo
resistencia_ja_projeto_final <- resistencia_jc + resistencia_cd + resistencia_da_final
sprintf("O valor de resistência máxima do dissipador do projeto deve ser: %.2f ohms.
        Considerando o valor de resistência do dissapador de calor de %.2f ohms o valor
        final da resistência Rja final será: %.2f ohms",resistencia_da,    resistencia_da_final, resistencia_ja_projeto_final);
```
```{r}
temperatura_juncao_final <- calculo_temperatura_juncao(potencia_conducao,resistencia_ja_projeto_final,temperatura_ambiente);
sprintf("Após a instalação do dissipador de calor modelo: WA-T220-101E a temperatura da junção ficou em:
        %.2f ºC", temperatura_juncao_final);
...
# 4.2.1 Cálculo Corrente Média e Corrente RMS:
...
corrente_rms_2 = sqrt((ciclo_trabalho_2*(corrente_ib_2^2 + corrente_ia_2*corrente_ib_2 + corrente_ia_2^2))/3);
sprintf("A corrente média no D1 será %.2f A e a corrente RMS no D1 será
        %.2f A", corrente_media_2, corrente_rms_2);
```
# 4.2.2 Cálculo da Potência Dissipada em Condução:

Os valores da resistência de condução $Rd$ e tensão direta $Vt$ do diodo VS - E5TH1506 - M3 foram obtidas no arquivo em anexo "Informações Diodo VS - E5TH1506 - M3".
...
sprintf("A potência dissipada em condução do diodo será: %.2f W", potencia_conducao_2);
```

# 4.2.3 Cálculo da Temperatura de Junção:
...
temperatura_juncao_2 <- calculo_temperatura_juncao(potencia_conducao_2,
                                                   resistencia_ja_2, temperatura_ambiente_2);
sprintf("A temperatura de junção do diodo será: %.2f ºC", temperatura_juncao_2);
```
Analisando o valor da temperatura de junção calculada $462.79^{\circ}C$ e o valor da temperatura máxima de junção do diodo que é $175^{\circ}C$ ve-se a necessidade de instalação de um dissipador de calor no projeto.

# 4.2.4 Cálculo do Dissipador de Calor:
...
resistencia_ja_projeto_2 <- calculo_resistencia_ja(temperatura_juncao_projeto_2,temperatura_ambiente_2, potencia_conducao_2);
resistencia_da_2 <- calculo_resistencia_da(resistencia_ja_projeto_2,resistencia_jc_2, resistencia_cd_2);
resistencia_da_final_2 <- 12;
resistencia_ja_projeto_final_2 <- (resistencia_jc_2 + resistencia_cd_2 + 
  resistencia_da_final_2);

sprintf("O valor de resistência máxima do dissipador do projeto deve ser: %.2f ohms.
        Considerando o valor de resistência do dissapador de calor de %.2f ohms o valor
        final da resistência Rja final será: %.2f ohms",resistencia_da_2,    resistencia_da_final_2, resistencia_ja_projeto_final_2);
```
```{r}
temperatura_juncao_final_2 <- calculo_temperatura_juncao(potencia_conducao_2, resistencia_ja_projeto_final_2, temperatura_ambiente_2);
sprintf("Após a instalação do dissipador de calor modelo: WA-T220-101E a temperatura da junção ficou em:
        %.2f ºC", temperatura_juncao_final_2);
```
